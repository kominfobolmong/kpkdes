...
<section class="bg-secondary pt-4 pb-4">
    <div class="container">
        ...
        <div class="text-center mt-4 mb-4">
            <h1 class="h2">PERTANIAN {{ $item->nama ? 'KEC.'.$item->nama : '' }}</h1>
        </div>
        <p>&nbsp;</p>
        <div class="row">
            <div class="col text-sm">
                <a data-id="0" href="{{ route('pertanian') }}" class="btn {{ $item->id ? 'btn-outline-info' : 'btn-info' }} text-white mb-2 mr-2">Semua</a>
                @foreach($kecamatans as $kecamatan)
                <a href="{{ route('pertanian', $kecamatan->slug) }}" class="btn {{ $item->id == $kecamatan->id ? 'btn-info' : 'btn-outline-info' }} text-white mb-2 mr-2">{{ $kecamatan->nama ?? null }}</a>
                @endforeach
            </div>
        </div>
        <p>&nbsp;</p>
            ...
            <div class="col text-center mt-4">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title text-sm">Produksi Pertanian</h3>
                    ...
                    <div class="card-body p-0 table-responsive">
                        <table class="table table-hover text-nowrap" style="width: 100%;">
                            ...
                            <tr>
                                ...
                                <td>{{ $item->luas_tanam ?? null }}</td>
                                <td>{{ $item->luas_panen ?? null }}</td>
                                <td>{{ $item->provitas ?? null }}</td>
                                <td>{{ $item->produksi ?? null }}</td>
                            </tr>
                            @endforeach
                        </table>
                    </div>
